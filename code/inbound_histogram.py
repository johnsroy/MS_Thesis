#!/bin/python
#python script used to draw a histogram test of inbound(downloaded) packets. 
...
for element in boxRecord:
	...
	mydata2.append(float(element[1]))
#print mydata1
#print mydata2
...
fig = plt.figure()
#fig= plt.histogram()
fig.suptitle('Probability Density Function', fontsize=14, fontweight='bold')
ax = fig.add_subplot(111)
fig.subplots_adjust(top=0.85)
ax.set_title('d2l.ucalgary.ca-packetsize')
'''
#bins = np.linspace(0,100,200)
#plt.hist(mydata1,bins=50,alpha=0.5,label='x')
plt.hist(mydata2,bins=100,alpha=0.5,range=(startvalue,endvalue), label='y')
